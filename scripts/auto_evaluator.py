...
class RAGEvaluator:
    def __init__(self, dataset_path: str, output_dir: str = "./eval_results", remarks: str = ""):
        ...
        self.eval_prompt = ChatPromptTemplate.from_messages([
            ("system", """あなたは「事実の正確性」を最優先する公平なAI評価者です。
提供された【ユーザーの質問】に対する【AIの回答】を、【模範解答】に含まれる「核となる事実情報」を基準にして1〜5点の5段階で評価してください。

[評価の重要ガイドライン]
1. 事実重視: 表現のゆらぎや資料間の細かな言い回しの違いは無視し、「伝えたい事実（日付、金額、条件、固有名詞など）」が一致しているかを最優先してください。
2. 形式の許容: AI回答がマークダウン（箇条書き、太字、表など）で構造化されており、模範解答がシンプルなテキストであっても、内容が同一であれば形式の差による減点は行わないでください。
3. 網羅性: AI回答が模範解答より詳しく、かつ正確である場合は「5点」としてください。
4. 根拠の妥当性: 参照した資料名が模範解答と異なっていても、その内容が客観的事実として正しい場合は減点しないでください。

[評価基準]
5点: 模範解答に含まれる重要な事実がすべて網羅されており、正確である。（構造化や詳細化による違いは不問）
4点: 主要な事実は捉えているが、判断に影響しない程度の軽微な情報が欠けている。
3点: 部分的に正しいが、核心的な事実の一部が欠落しているか、回答が曖昧である。
2点: 関連するキーワードは含まれているが、質問の意図に対して不正確、または的を射ていない。
1点: 事実関係が完全に誤っているか、全く無関係な回答である。"""),
("human", "[質問]\n{query}\n\n[模範解答]\n{reference_answer}\n\n[AI回答]\n{actual_answer}")
        ])
